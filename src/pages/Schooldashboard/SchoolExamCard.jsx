...
import { Box, Button, Grid, Paper, Typography , Dialog, DialogActions, DialogContent, DialogTitle, TextField, List, ListItem, ListItemText , MenuItem ,FormControl , InputLabel , Select  } from '@mui/material';
...
const ExamCard = () => {
  ...
  const fetchStudentData = async () => {
    const { data: studentData, error: studentError } = await fetchData('SELECT * FROM students'); // Replace with your actual SQL query
  ...
  {examData?.length > 0 ? (
    examData.map((exam) => (
     ...
     <div className='postedBy'>
      <span>Posted By</span><br></br>
      {exam.teacheremail}
      </div>
     </div>

     <div className='space' id='bold' >
        {exam.title}
     </div>


     <div className='space' >
     Start Date: {new Date(exam.exam_date).toLocaleDateString()}<br></br>
     ...
     <Button
            ...
            sx={{
              ...
              display: 'block', // Make sure buttons stack
              width: '100%',
            }}
          >
            View TimeTable
