...
import {
  Box, Avatar, Typography, Button, Card, CardContent, CardActions, Grid, Link, Select, MenuItem,
  TextField, Dialog, DialogActions, DialogContent, DialogTitle, Divider,
} from '@mui/material';
...
const AllTeachers = () => {
  ...
  useEffect(() => {
    ...
    const fetchDataAsync = async () => {
      ...
      const { data: fetchedData, error: fetchError } = await fetchData(`SELECT * FROM teachers where schoolcode= "${myschoolCode}" `);
  ...
  const classes = ['All', 'Class 12', 'Class 11', 'Class 10', 'Class 9', 'Class 8', 'Class 7', 'Class 6', 'Class 5'];
  ...
  const filteredData = useMemo(() => {
    if (searchTerm == 'all') {
      ...
      return data.filter((item) =>
        `${item.first_name?.toLowerCase() || ''} ${item.last_name?.toLowerCase() || ''}`.includes(searchTerm?.toLowerCase() || '')
      );
    }
  }, [data, searchTerm]);
